...
class WilliamsPercentRSpec extends FlatSpec with Matchers {
  val testData = Table(
  ...
  "WilliamsPercentR" should "produce the correct results" in {
    ...
    forAll(testData) {(high: Double,
                       ...
                       williamsPercentR: Option[Double]) =>

      val data = TickData.getNextTick(Range(high, low), close, 0.0, prevData, williamsPercentR = true)
      data.williamsPercentR match {
        case Some(x) =>
          TestHelpers.matchOptionDouble(x.highestHigh, highestHigh, 0.0001)
          TestHelpers.matchOptionDouble(x.lowestLow, lowestLow, 0.0001)
          TestHelpers.matchOptionDouble(x.percentR, williamsPercentR, 0.0000001)
        case None => fail()
      }
