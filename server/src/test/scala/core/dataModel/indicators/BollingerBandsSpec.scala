...
class BollingerBandsSpec  extends FlatSpec with Matchers {
  val testData = Table(
  ...
  "BollingerBands" should "produce the correct figures" in {
    ...
    forAll(testData) { (close: Double, middle: Option[Double], stdDev: Option[Double], upper: Option[Double], lower: Option[Double], bandWidth: Option[Double]) =>
      val data = TickData.getNextTick(Range(0, 0), close, 0.0, prevData, bollingerBands = true)
      data.bollingerBands match {
        case Some(x) =>
          TestHelpers.matchOptionDouble(x.middle, middle, 0.000001)
          TestHelpers.matchOptionDouble(x.stdDev, stdDev, 0.000000001)
          TestHelpers.matchOptionDouble(x.upper, upper, 0.00000001)
          TestHelpers.matchOptionDouble(x.lower, lower, 0.00000001)
          TestHelpers.matchOptionDouble(x.bandWidth, bandWidth, 0.000000001)
        case None => fail()
      }
