...
class EaseOfMovementSpec extends FlatSpec with Matchers {
  ...
  "EaseOfMovement" should "produce the correct figures" in {
    ...
    forAll(testData) {(high: Double, low: Double, volume: Double, _emv: Option[Double], smaOfEMV: Option[Double]) =>
      val data = TickData.getNextTick(Range(high, low), 0.0, volume, prevData, easeOfMovement = true)
      data.easeOfMovement match {
        case Some(x) =>
          TestHelpers.matchOptionDouble(x.emv, _emv, 0.000000001)
          TestHelpers.matchOptionDouble(x.smaOfEMV, smaOfEMV, 0.000000001)
        case None => fail()
      }
