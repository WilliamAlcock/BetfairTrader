...
class QATSpec extends FlatSpec with Matchers{
  ...
  "getMean and getSimpleMovingAvergae" should "should produce the same figures" in {
    ...
    for (data <- testData) {
      ...
      val mean: Option[Double] = prevData.size match {
        case x if x >= period => Some(QAT.getMean(prevData.take(period)))
        case _ => None
      }
      val sma: Option[Double] = prevSMA match {
        case Some(x) =>
          Some(QAT.getSimpleMovingAverage(period, data, prevData(period), x))
        case _ => None
      }
      (mean, sma) match {
        case (Some(x), Some(y)) =>
