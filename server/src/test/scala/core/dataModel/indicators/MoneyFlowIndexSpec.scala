...
class MoneyFlowIndexSpec extends FlatSpec with Matchers {
  val testData = Table(
  ...
  "MoneyFlowIndex" should "produce the correct figures" in {
    ...
    forAll(testData) {(typicalPrice: Double,
      ...
      val data = TickData.getNextTick(Range(0, 0), close, volume, prevData, typicalPrice = Some(typicalPrice), moneyFlowIndex = true)
      data.typicalPrice should be (typicalPrice)
      data.moneyFlowIndex match {
        case Some(x) =>
          TestHelpers.matchOptionDouble(x.rawFlow, rawFlow, 0.0001)
          TestHelpers.matchOptionDouble(x.posFlow, posFlow, 0.0001)
          TestHelpers.matchOptionDouble(x.negFlow, negFlow, 0.0001)
          TestHelpers.matchOptionDouble(x.avgPosFlow, avgPosFlow, 0.001)
          TestHelpers.matchOptionDouble(x.avgNegFlow, avgNegFlow, 0.001)
          TestHelpers.matchOptionDouble(x.ratio, ratio, 0.00000001)
          TestHelpers.matchOptionDouble(x.index, index, 0.00000001)
        case None => fail()
      }
