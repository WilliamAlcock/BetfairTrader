...
class CommodityChannelIndexSpec extends FlatSpec with Matchers {
  ...
  "CommodityChannelIndex" should "produce the correct figures" in {
    ...
    forAll(testData) {(high: Double, low: Double, close: Double, typicalPrice: Double, smaOfTP: Option[Double], cciMeanDeviation: Option[Double], cci: Option[Double]) =>
      val data = TickData.getNextTick(Range(high, low), close, 0.0, prevData, commodityChannelIndex = true)
      data.typicalPrice should be (typicalPrice +- 0.0000001)
      data.commodityChannelIndex match {
        case Some(x) =>
          TestHelpers.matchOptionDouble(x.smaOfTypicalPrice, smaOfTP, 0.0000001)
          TestHelpers.matchOptionDouble(x.meanDeviation, cciMeanDeviation, 0.0000001)
          TestHelpers.matchOptionDouble(x.index, cci, 0.0000001)
        case None => fail()
      }
