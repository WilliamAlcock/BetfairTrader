...
import domain.{Runner, ExchangePrices, PriceSize}
...
case class CSVData(eventId: String,
  ...
  def toRunner(): Runner = Runner(
    selectionId,
    0.0,
    status,
    lastPriceTraded = lastPriceMatched,
...
object CSVData {
  ...
  val timeStampFormat = DateTimeFormat.forPattern("yyyy-MM-ddHH:mm:ss.SSS").withZone(DateTimeZone.UTC)
  ...
  def fromCSVString(s: String): CSVData = {
    ...
    CSVData(
      ...
      cols(5),                                                    // raceCourse
      DateTime.parse(cols(3) + cols(6), timeStampFormat),         // timestamp
