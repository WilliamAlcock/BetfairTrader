...
class StochasticOscillatorSpec extends FlatSpec with Matchers with TestHelpers {
  val testData = Table(
  ...
  case class Data(tick: Tick, stochasticOscillator: StochasticOscillator) extends StochasticOscillatorData
  ...
  "StochasticOscillator" should "produce the correct figues" in {
    ...
    forAll(testData) {(high: Double,
      ...
      val stochasticOscillator = StochasticOscillator.getNext(tick, prevData)
      matchOptionDouble(stochasticOscillator.highestHigh, highestHigh, 0.0001)
      matchOptionDouble(stochasticOscillator.lowestLow, lowestLow, 0.0001)
      matchOptionDouble(stochasticOscillator.percentK, stochasticOscillatorK, 0.00000001)
      matchOptionDouble(stochasticOscillator.percentD, stochasticOscillatorD, 0.00000001)
      prevData = Data(tick, stochasticOscillator) :: prevData
    }
  }
}
