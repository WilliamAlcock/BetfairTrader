...
class RelativeStrengthIndexSpec extends FlatSpec with Matchers with TestHelpers {
  val testData = Table(
  ...
  case class Data(tick: Tick, relativeStrengthIndex: RelativeStrengthIndex) extends RelativeStrengthIndexData
  ...
  "RelativeStrengthIndex" should "produce the correct figures" in {
    ...
    forAll(testData) { (close: Double,
      ...
      val relativeStrengthIndex = RelativeStrengthIndex.getNext(tick, prevData)
      matchOptionDouble(tick.closeDelta, closeDelta, 0.0001)
      matchOptionDouble(relativeStrengthIndex.gain, gain, 0.0001)
      matchOptionDouble(relativeStrengthIndex.loss, loss, 0.0001)
      matchOptionDouble(relativeStrengthIndex.avgGain, avgGain, 0.000000001)
      matchOptionDouble(relativeStrengthIndex.avgLoss, avgLoss, 0.000000001)
      matchOptionDouble(relativeStrengthIndex.rs, rs, 0.000000001)
      matchOptionDouble(relativeStrengthIndex.rsi, rsi, 0.00000001)
      prevData = Data(tick, relativeStrengthIndex) :: prevData
    }
  }
}
