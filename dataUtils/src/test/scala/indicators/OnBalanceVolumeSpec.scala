...
class OnBalanceVolumeSpec extends FlatSpec with Matchers {
  ...
  case class Data(tick: Tick, onBalanceVolume: Double) extends OnBalanceVolumeData

  "OnBalanceVolume" should "produce the correct figures" in {
    var prevData = List.empty[Data]

    forAll(testData) {(close: Double, volume: Double, obv: Double) =>
      val tick = Tick.getNext(close, Range(0, 0), volume, 0.0, prevData)
      val onBalanceVolume = OnBalanceVolume.getNext(tick, prevData)
      onBalanceVolume should be(obv +- 0.1)
      prevData = Data(tick, onBalanceVolume) :: prevData
    }
  }
}